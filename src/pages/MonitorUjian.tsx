...
export default function MonitorUjian() {
  ...
  const handleUnlockParticipant = async (participantId: string, participantName: string) => {
    ...
    try {
      ...
      fetchExamAndParticipants();
      alert(`Akun ${participantName} berhasil dibuka! Peserta dapat melanjutkan ujian.`);
    } catch (err) {
  ...
  const handleResetParticipant = async (participantId: string, participantName: string) => {
    const confirmed = window.confirm(
      `Apakah Anda yakin ingin me-reset ujian untuk "${participantName}"?\n\n` +
      `• Jawaban yang telah dikirim akan dihapus.\n` +
      `• Status selesai/terblokir akan dikembalikan ke awal.\n` +
      `• Waktu (timer) pengerjaan akan dimulai ulang dari awal.\n` +
      `• Pelanggaran akan di-reset menjadi 0.\n\n` +
      `Siswa dapat masuk kembali dan mengerjakan soal dari nomor 1.`
    );
    if (!confirmed) return;

    try {
      // 1. Delete answers (with dual client support)
      try {
      ...
      // 3. Broadcast reset event to active presence room
      if (roomChannelRef.current) {
        try {
          await roomChannelRef.current.send({
            type: 'broadcast',
          ...
          console.warn('Broadcast reset error:', bErr);
        }
      }

      fetchExamAndParticipants();
      alert(`Ujian untuk "${participantName}" berhasil di-reset! Siswa dapat masuk kembali dan mulai mengerjakan dari awal.`);
    } catch (err: any) {
      console.error('Error resetting participant:', err);
      alert('Gagal me-reset ujian peserta: ' + (err.message || 'Terjadi kesalahan'));
    }
  };
                  ...
                  {status.type === 'blocked' && (
                    <div className="flex flex-col gap-1.5 mt-2">
                      ...
                      >
                        ...
                        <span>Reset Total (Mulai Ulang)</span>
                      </button>
                    </div>
                  )}

                  {p.hasJoined && status.type !== 'completed' && status.type !== 'blocked' && (
                    <button
                      onClick={() => handleResetParticipant(p.id, p.name)}
                      className="w-full mt-1 bg-slate-50 hover:bg-slate-100 text-slate-500 hover:text-slate-700 border border-slate-200/70 px-2.5 py-1.5 rounded-lg text-[10px] font-bold flex items-center justify-center gap-1.5 transition-all cursor-pointer opacity-70 hover:opacity-100"
                      title="Reset ujian siswa ini dari awal jika mengalami kendala"
                    >
