...
export const Attendance: React.FC<Props> = ({ state, refresh, notify }) => {
  ...
  useEffect(() => {
    const active = isScanning || isFaceScanning;
    window.dispatchEvent(new CustomEvent('scan_mode_change', { detail: { active } }));
  ...
  const handleCloseSession = async () => {
      ...
      const ok = await notifier.confirm({ 
          title: "Tutup Sesi Absensi", 
          message: unscannedStudents.length > 0
            ? `Tutup sesi absensi ini dan tandai ${unscannedStudents.length} siswa yang belum absen sebagai ALPHA?`
            : "Tutup sesi absensi ini? Setelah ditutup, siswa tidak dapat melakukan absensi lagi."
      });
      ...
      if (ok) {
          ...
          const updatedSession: AttendanceSession = { ...currentSession, isClosed: true };
          await upsertSession(updatedSession);
          setCurrentSession(updatedSession);
          
          await refresh(); 
          notify("Sesi ditutup & dikunci. Siswa tidak dapat melakukan scan absensi lagi.", "success");
      }
  };

  const handleReopenSession = async () => {
      if (!currentSession) return;
      const updatedSession: AttendanceSession = { ...currentSession, isClosed: false };
      await upsertSession(updatedSession);
      setCurrentSession(updatedSession);
      await refresh();
      notify("Sesi absensi dibuka kembali.", "success");
  };
...
const switchCamera = async () => {
  ...
  const processScan = async (qrData: string) => {
    ...
    if (currentSession?.isClosed) {
        setScanFeedback({ type: 'error', title: 'Absensi Ditutup', message: 'Sesi absensi ini telah ditutup oleh guru.', timestamp: now });
        playErrorSound();
        triggerFeedbackPause();
        return;
    }
  ...
  const handleFaceAttendanceSuccess = async (studentId: string, studentName: string, status: AttendanceStatus) => {
    ...
    if (currentSession?.isClosed) {
      setScanFeedback({
        ...
        message: 'Sesi absensi ini telah ditutup oleh guru.',
        timestamp: Date.now()
      });
      playErrorSound();
  ...
  if (!activeClass) {
    return (
      <div className="space-y-6">
        <Header title="Scan Kehadiran" subtitle="Pilih kelas untuk mulai absensi." />
        ...
        <div className="flex-1 flex items-center justify-center relative">
          <div className="relative w-72 h-72 md:w-96 md:h-96">
            ...
            <div className="absolute top-0 left-0 w-12 h-12 md:w-16 md:h-16">
              <div className="w-full h-full border-l-4 border-t-4 border-blue-400 rounded-tl-xl" />
            </div>
            <div className="absolute top-0 right-0 w-12 h-12 md:w-16 md:h-16">
              <div className="w-full h-full border-r-4 border-t-4 border-blue-400 rounded-tr-xl" />
            </div>
            <div className="absolute bottom-0 left-0 w-12 h-12 md:w-16 md:h-16">
              <div className="w-full h-full border-l-4 border-b-4 border-blue-400 rounded-bl-xl" />
            </div>
            <div className="absolute bottom-0 right-0 w-12 h-12 md:w-16 md:h-16">
              <div className="w-full h-full border-r-4 border-b-4 border-blue-400 rounded-br-xl" />
            </div>
            ...
            {scanFeedback ? (
              <motion.div 
                ...
                className={`max-w-lg mx-auto rounded-2xl p-5 shadow-2xl border-t-4 backdrop-blur-xl ${
                  scanFeedback.type === 'success' ? 'bg-gradient-to-r from-blue-500 to-blue-600 text-white border-blue-400 shadow-blue-500/40' :
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div className="flex flex-wrap gap-2">
          {activeSchedule ? (
              ...
              <div className="flex items-center gap-2 text-gray-400 bg-gray-100 px-3 py-1 rounded-lg text-xs font-bold w-fit">
          ...
          {currentSession?.isClosed ? (
              <div className="flex items-center gap-2 text-red-650 bg-red-50 border border-red-200 px-3 py-1 rounded-lg text-xs font-bold w-fit">
                 <XCircle className="w-4 h-4 text-red-600" />
                 Status: Ditutup & Dikunci
              </div>
          ) : currentSession ? (
              <div className="flex items-center gap-2 text-emerald-650 bg-emerald-50 border border-emerald-200 px-3 py-1 rounded-lg text-xs font-bold w-fit">
